...
v = @SVector rand(3)
q = rand(QuatRotation)
ForwardDiff.jacobian(q -> QuatRotation(q, false) * v, Rotations.params(q)) ≈ Rotations.∇rotate(q, v)
...
RobotDynamics.inertia(::Body) = Diagonal(SA[2, 3, 1.0])
...
@test length(u) == 6
@test norm(x[4:7]) ≈ 1
@test RobotDynamics.LieState(model) === RobotDynamics.LieState(QuatRotation{Float64}, (3, 6))
@test RobotDynamics.rotation_type(model) == QuatRotation{Float64}
...
for R in [QuatRotation{Float64}, MRP{Float64}, RodriguesParam{Float64}]
    local model = Body{R}()
...
@test RobotDynamics.fill_state(model, 1, 2, 3, 4.0) isa SVector{13,Float64}
...
@test RobotDynamics.fill_state(model2, 1, 2, 3, 4.0)[4:6] ≈ fill(2, 3)
